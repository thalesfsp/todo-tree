...
| todo-tree.general.rootFolder | <tt>""</tt> | By default, any open workspaces will have a tree in the view. Use this to force another folder to be the root of the tree. You can include environment variables and also use ${workspaceFolder}. e.g. <br>`"todo-tree.rootFolder": "${workspaceFolder}/test"`<br>or<br>`"todo-tree.rootFolder": "${HOME}/project"`<br> *Note: Other open files (outside of the rootFolder) will be shown (as they are opened) with their full path in brackets.* |
